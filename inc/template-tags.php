...
/**
 * Decorative wavy SVG divider between contrasting sections.
 *
 * @param string $direction 'down' (curve dips down, default) or 'up'.
 * @param string $class Extra wrapper class, e.g. to set fill color via CSS.
 */
function yhdr_wave_divider( $direction = 'down', $class = '' ) {
	$path = $direction === 'up'
		? 'M0,32 C240,90 480,90 720,60 C960,30 1200,0 1440,32 L1440,90 L0,90 Z'
		: 'M0,58 C240,0 480,0 720,30 C960,60 1200,90 1440,58 L1440,0 L0,0 Z';
	?>
...
function yhdr_render_project_card( $post, $variant = 'grid' ) {
	...
	?>
	<article class="project-card project-card--<?php echo esc_attr( $variant ); ?> card" <?php echo $tag ? 'data-category="' . esc_attr( sanitize_title( $tag ) ) . '"' : ''; ?>>
		...
		<div class="project-card__body">
			...
			<?php endif; ?>
			<a href="<?php echo esc_url( get_permalink( $post ) ); ?>" class="project-card__link" aria-label="<?php echo esc_attr( get_the_title( $post ) ); ?>">&rarr;</a>
		</div>
	</article>
	<?php
}
...
function yhdr_render_testimonial_slide( $fields ) {
	...
	<div class="testimonial-card">
		<div class="testimonial-card__rating" aria-hidden="true"><?php echo esc_html( str_repeat( '★', $rating ) . str_repeat( '☆', 5 - $rating ) ); ?></div>
